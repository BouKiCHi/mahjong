...
class App:
    ...
    def reset_card(self):
        ...
        # 1段目タイル
        tile = [0] * (16*12)
        self.fill_plane(3, 2, 10, 8, card, tile)
        tiles.append(tile)

        # 2段目タイル
        tile = [0] * (16*12)
        self.fill_plane(5, 3, 6, 6, card, tile)
        tiles.append(tile)

        # 3段目タイル
        tile = [0] * (16*12)
        self.fill_plane(6, 4, 4, 4, card, tile)
        tiles.append(tile)

        # 4段目タイル
        tile = [0] * (16*12)
        self.fill_plane(7, 5, 2, 2, card, tile)
        tiles.append(tile)
        self.tiles = tiles

    # 面を埋める
    def fill_plane(self, xofs:int, yofs:int, w:int, h:int, card:Card, tile):
        for yi in range(h):
            for xi in range(w):
    ...
    # 上が空いているか
    def is_top_empty(self, plane_no, pi):
        for i in range(plane_no+1, 4):
            if self.tiles[i][pi] != 0:
                return False
        return True
